...
Nexora — Discord-подобное приложение:
...
### 3.1. Клонирование
```bash
git clone https://github.com/YuriiFridman/Nexora.git
cd Nexora
```
...
DATABASE_URL=postgresql+asyncpg://user:pass@localhost:5432/nexora
...
Установить в `.env`:
```env
DATABASE_URL=postgresql+asyncpg://postgres:postgres@localhost:5432/nexora
```
...
| Вопрос | Ответ |
|--------|-------|
| What is your app name? | `Nexora` |
| What should the window title be? | `Nexora` |
...
- **Windows**: `desktop/src-tauri/target/release/bundle/msi/*.msi`
  и/или `desktop/src-tauri/target/release/nexora.exe`
...
2. **Deploy from GitHub repo** → выберите `YuriiFridman/Nexora`
...
| `CORS_ORIGINS` | Домен фронтенда (например `https://nexora.vercel.app`) |
